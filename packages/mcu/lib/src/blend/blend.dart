...
abstract final class Blend {
  static int harmonize(int designColor, int sourceColor) {
    ...
    final differenceDegrees = math_utils.differenceDegrees(
      fromHct.hue,
      toHct.hue,
    );
    final rotationDegrees = math.min(differenceDegrees * 0.5, 15.0);
    final outputHue = math_utils.sanitizeDegreesDouble(
      fromHct.hue +
          rotationDegrees *
              math_utils.rotationDirection(fromHct.hue, toHct.hue),
    );
  ...
  static int hctHue(int from, int to, double amount) {
    ...
    final blended = Hct.from(
      ucsCam.hue,
      fromCam.chroma,
      color_utils.lstarFromArgb(from),
    );
