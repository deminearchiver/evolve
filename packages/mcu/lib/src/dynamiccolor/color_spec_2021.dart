...
import 'package:mcu/src/temperature/temperature_cache.dart';
...
class ColorSpec2021 implements ColorSpec {
  ...
  double getTone(DynamicScheme scheme, DynamicColor color) {
    ...
    if (toneDeltaPair != null) {
      ...
      if ((fTone - nTone) * expansionDir < delta) {
        ...
        fTone = math_utils.clampDouble(0, 100, nTone + delta * expansionDir);
        // If constraint is not satisfied, try another round.
        if ((fTone - nTone) * expansionDir < delta) {
          // 3rd round: contract nearer to match delta.
          nTone = math_utils.clampDouble(0, 100, fTone - delta * expansionDir);
        }
      }
  ...
  TonalPalette getPrimaryPalette(
    ...
    return switch (variant) {
      ...
      Variant.fruitSalad => TonalPalette.fromHueAndChroma(
        math_utils.sanitizeDegreesDouble(sourceColorHct.hue - 50.0),
        48.0,
      ),
      ...
      Variant.rainbow => TonalPalette.fromHueAndChroma(
        sourceColorHct.hue,
        48.0,
      ),
      ...
      Variant.expressive => TonalPalette.fromHueAndChroma(
        math_utils.sanitizeDegreesDouble(sourceColorHct.hue + 240),
        40,
      ),
    ...
    return switch (variant) {
      Variant.content => TonalPalette.fromHct(
        DislikeAnalyzer.fixIfDisliked(
          TemperatureCache(sourceColorHct).generateAnalogousColors(3, 6)[2],
        ),
      ),
      ...
      Variant.rainbow || Variant.tonalSpot => TonalPalette.fromHueAndChroma(
        math_utils.sanitizeDegreesDouble(sourceColorHct.hue + 60.0),
        24.0,
      ),
      Variant.expressive => TonalPalette.fromHueAndChroma(
        DynamicScheme.getRotatedHue(
  ...
  TonalPalette getNeutralPalette(
    ...
    return switch (variant) {
      ...
      Variant.expressive => TonalPalette.fromHueAndChroma(
        math_utils.sanitizeDegreesDouble(sourceColorHct.hue + 15),
        8,
      ),
