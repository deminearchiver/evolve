...
import 'package:mcu/src/dynamiccolor/color_spec_2021.dart';
import 'package:mcu/src/dynamiccolor/dynamic_color.dart';
import 'package:mcu/src/dynamiccolor/dynamic_scheme.dart';
import 'package:mcu/src/dynamiccolor/tone_delta_pair.dart';
import 'package:mcu/src/dynamiccolor/variant.dart';
import 'package:mcu/src/hct/hct.dart';
...
class ColorSpec2025 extends ColorSpec2021 {
  ...
  static double _tMaxC(
    ...
    return math_utils.clampDouble(lowerBound, upperBound, answer);
  }
  ...
  static double _getExpressiveNeutralChroma(
    ...
    double neutralHue = _getExpressiveNeutralHue(sourceColorHct);
    bool neutralHueIsYellow = neutralHue >= 105 && neutralHue < 125;
  ...
  static double _getVibrantNeutralChroma(
    ...
    final neutralHueIsBlue = neutralHue >= 250 && neutralHue < 270;
    return platform == Platform.phone ? 28 : (neutralHueIsBlue ? 28 : 20);
  }
  ...
  DynamicColor onSurface() {
    final color2025 = DynamicColor.fromPalette(
      ...
      contrastCurve: (s) => _getContrastCurve(9),
    );
      ...
      tone: (s) {
        if (s.variant == Variant.neutral) {
          ...
          return _tMaxC(
            s.primaryPalette,
            0,
            s.primaryPalette.keyColor.isYellow
                ? 25
                : s.primaryPalette.keyColor.isCyan
                ? 88
                : 98,
          );
        } else {
          ...
          return _tMaxC(
            s.primaryPalette,
            0,
            s.primaryPalette.keyColor.isCyan ? 88 : 98,
          );
        }
      },
      isBackground: true,
        ...
        if (s.platform == Platform.watch) {
          ...
          return s.isDark
              ...
              : _tMaxC(
                  s.primaryPalette,
                  78,
                  s.primaryPalette.keyColor.isCyan ? 88 : 90,
                );
        } else {
          // VIBRANT
          return s.isDark
              ...
              : _tMaxC(
                  s.primaryPalette,
                  66,
                  s.primaryPalette.keyColor.isCyan ? 88 : 93,
                );
        }
      },
      isBackground: true,
      ...
      tone: (s) {
        if (s.platform == Platform.watch) {
          ...
          return _tMaxC(
            s.tertiaryPalette,
            /* lowerBound= */ 0,
            /* upperBound= */ s.tertiaryPalette.keyColor.isCyan
                ? 88
                : (s.isDark ? 98 : 100),
          );
        } else {
    ...
    if (toneDeltaPair != null) {
      ...
      switch (constraint) {
        case DeltaConstraint.exact:
          selfTone = math_utils.clampDouble(
            0,
            100,
      ...
      if (color.isBackground && !color.name.endsWith("_fixed_dim")) {
        if (selfTone >= 57) {
          selfTone = math_utils.clampDouble(65, 100, selfTone);
        } else {
          selfTone = math_utils.clampDouble(0, 49, selfTone);
        }
      }
      ...
      if (color.isBackground && !color.name.endsWith("_fixed_dim")) {
        if (answer >= 57) {
          answer = math_utils.clampDouble(65, 100, answer);
        } else {
          answer = math_utils.clampDouble(0, 49, answer);
        }
      }
