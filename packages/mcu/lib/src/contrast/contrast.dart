...
abstract final class Contrast {
  ...
  static double ratioOfTones(double t1, double t2) {
    return ratioOfYs(color_utils.yFromLstar(t1), color_utils.yFromLstar(t2));
  }
  ...
  static double lighter(double tone, double ratio) {
    ...
    final double darkY = color_utils.yFromLstar(tone);
    ...
    final double returnValue =
        color_utils.lstarFromY(lightY) + _luminanceGamutMapTolerance;
  ...
  static double darker(double tone, double ratio) {
    ...
    final double lightY = color_utils.yFromLstar(tone);
    ...
    final double returnValue =
        color_utils.lstarFromY(darkY) - _luminanceGamutMapTolerance;
    // NOMUTANTS--important validation step; functions it is calling may change implementation.
    if (returnValue < 0 || returnValue > 100) {
      return -1.0;
    }
