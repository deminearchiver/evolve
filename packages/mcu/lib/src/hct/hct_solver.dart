...
abstract final class HctSolver {
  ...
  static double _hueOf(List<double> linrgb) {
    final scaledDiscount = math_utils.matrixMultiply(
      linrgb,
  ...
  static List<double> _midpoint(List<double> a, List<double> b) {
    return <double>[(a[0] + b[0]) / 2, (a[1] + b[1]) / 2, (a[2] + b[2]) / 2];
  }
  ...
  static double _inverseChromaticAdaptation(double adapted) {
    double adaptedAbs = adapted.abs();
    double base = math.max(0, 27.13 * adaptedAbs / (400.0 - adaptedAbs));
    return math_utils.signum(adapted) * math.pow(base, 1.0 / 0.42).toDouble();
  }
  ...
  static int _findResultByJ(double hueRadians, double chroma, double y) {
    ...
    for (int iterationRound = 0; iterationRound < 5; iterationRound++) {
      ...
      final linrgb = math_utils.matrixMultiply([
        rCScaled,
        gCScaled,
        bCScaled,
      ...
      if (iterationRound == 4 || (fnj - y).abs() < 0.002) {
        ...
        return color_utils.argbFromLinrgb(linrgb);
      }
  ...
  static int solveToInt(double hueDegrees, double chroma, double lstar) {
    if (chroma < 0.0001 || lstar < 0.0001 || lstar > 99.9999) {
      return color_utils.argbFromLstar(lstar);
    }
    hueDegrees = math_utils.sanitizeDegreesDouble(hueDegrees);
    final hueRadians = hueDegrees / 180 * math.pi;
    final y = color_utils.yFromLstar(lstar);
